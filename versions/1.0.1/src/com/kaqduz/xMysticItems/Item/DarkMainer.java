...
public HashMap<String, Long> MainerCooldown = new HashMap<String, Long>();
public boolean CanUseAgain(Player p){
	if(!p.isOp())
		return true;
int cooldownTime = 2;
if(MainerCooldown.containsKey(p.getName())) {

long secondsLeft = ((MainerCooldown.get(p.getName())/1000)+cooldownTime) - (System.currentTimeMillis()/1000);
...
MainerCooldown.put(p.getName(), System.currentTimeMillis());
return true;
}
